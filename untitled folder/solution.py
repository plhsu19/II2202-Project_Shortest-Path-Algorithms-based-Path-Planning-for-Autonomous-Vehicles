...
class Node(object):
    ...
    def set_cost(self):
        ...
        self.cost = 1.0 * self.costG + 1.0 * self.costH
    ...
    for dir in steering: #for all δ do
        ...
        if not (n_node_idx in closed_set):
            ...
            if k == 404:
                ...
                #compute new costs g′
                n_node.costG = c_node.costG + 0.01 * timesteps #(distance/timestep = 0.01, velocity = 1)
                ...
                if n_node.cost < open_set[n_node_idx].cost:
                    #print("hit update path")
                    open_set[n_node_idx] = n_node
            #else
            elif k == (timesteps - 1):
                # O ← O ∪ {n′}
                n_node.costG = c_node.costG + 0.01 * timesteps 
                n_node.set_cost() #initialize cost_H and cost
                n_node.parent = c_node
                n_node.time = c_node.time + timesteps
                n_node.phi = dir
...
def solution(car):
    ...
    while len(open_set) != 0:
       ...
       if c_node.verify_goal() or c >= 1000000:
           if c_node.verify_goal():
               print("Find goal")
               print("c_node x, y = ", c_node.x, ", ", c_node.y)
               print("path distance = ", c_node.costG)
               print("length of open_set = ", len(open_set))
               print("length of closed set = ", len(closed_set))
           else:
    ...
    if len(open_set) == 0:
        print("failed to reach target before running out of the states in open set")
        print("c_node x, y, theta = ", c_node_r.x, ", ", c_node_r.y, ", ", c_node_r.theta)
        print("path distance = ", c_node_r.costG)
        print("length of open_set = ", len(open_set))
        print("length of closed set = ", len(closed_set))
        times.insert(0, c_node_r.time * dt)
        controls.insert(0, c_node_r.phi)
        #trace back until there is no parent node, ie, current node is the start node
        while True:
            if c_node_r.parent == None:
               controls.pop(0)
               break
            else:
                c_node_r = c_node_r.parent
                times.insert(0, c_node_r.time * dt)
                controls.insert(0, c_node_r.phi) # control needs to be poped out if c_node is the start node
